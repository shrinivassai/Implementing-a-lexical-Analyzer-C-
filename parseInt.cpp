...
// • extern bool LogANDExpr(istream& in, int& line, Value & retVal); GOOD
// • extern bool Expr(istream& in, int& line, Value & retVal); GOOD
...
bool Expr(istream& in, int& line, Value & retVal) {
	...
	//cout << "status of logANDExpr and val1: " << t1<< " " << val1.IsErr() << " " << val1.IsBool() << endl;
	//cout << "status of logANDExpr: " << t1<< endl;
		
	if( !t1 ) {
		return false;
	}
	//cout << "value of var1 in OR op " << val1.GetBool() << endl;
	retVal = val1;
	tok = Parser::GetNextToken(in, line);
	if(tok.GetToken() == ERR){
		ParseError(line, "Unrecognized Input Pattern");
